...
uint8_t I2C_WaitFlagStatusUntilTimeout(I2C_TypeDef *I2Cx, uint32_t I2C_FLAG, FlagStatus Status, uint32_t Timeout)
{   
    TIM2->CNT = 0;
    uint16_t InitTick = TIM2->CNT;

    while (I2C_GetFlagStatus(I2Cx, I2C_FLAG) != Status)
    {
        if (Timeout <= I2C_MAX_TIMEOUT)
        {
            if (TIM2->CNT - InitTick >= Timeout)
            {
...
uint8_t I2C_WaitCheckEventUntilTimeout(I2C_TypeDef *I2Cx, uint32_t I2C_EVENT, uint32_t Timeout)
{   
    TIM2->CNT = 0;
    uint16_t InitTick = TIM2->CNT;
