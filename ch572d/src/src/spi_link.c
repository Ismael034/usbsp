...
__attribute__((aligned(4))) static uint8_t spi_usb_info_req[SPI_LINK_FRAME_LEN];
...
void spi_link_get_active_usb_info(uint16_t offset, uint8_t req_len, uint8_t *rx, uint8_t len)
{
    ...
    spi_usb_info_req[1] = (uint8_t)(offset & 0xFFU);
    spi_usb_info_req[2] = (uint8_t)((offset >> 8) & 0xFFU);
    spi_usb_info_req[3] = req_len;

    memset(rx, 0, rx_len);
    spi_link_exchange(spi_usb_info_req, rx, rx_len, 1000);
}
